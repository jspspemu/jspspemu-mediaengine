
//console.log('hello');

//console.log(_main);
//console.log(_main2());

var AudioDecoder = function() {
}

AudioDecoder.prototype.init = function() {
	this.id = _me_audio_decode_alloc(10);
};

AudioDecoder.prototype.getNumSamples = function() {
	return _me_audio_decode_get_numsamples(this.id);
};

AudioDecoder.prototype.getNumChannels = function() {
	return _me_audio_decode_get_numchannels(this.id);
};

AudioDecoder.prototype.setData = function(data) {
	var temp = _malloc(data.length);
	writeArrayToMemory(data, temp);
	_me_audio_decode_set_data(this.id, temp, data.length);
	_free(temp);
};

AudioDecoder.prototype.getSample = function(channel, sample) {
	return _me_audio_decode_get_sample(this.id, channel, sample);
};
 
AudioDecoder.prototype.deinit = function() {
	_me_audio_decode_free(this.id);
};

var compressedData = new Uint8Array([
     
                                                                                                                                            ]);
                                                                                                                                            

var ad = new AudioDecoder();
ad.init();
ad.setData(compressedData);
var samples = ad.getNumSamples();
var channels = ad.getNumChannels();
for (var n = 0; n < samples; n++) {
	for (var ch = 0; ch < channels; ch++) {
		console.log(ad.getSample(ch, n));
	}
}
ad.deinit();
